"""R2StorageService: Cloudflare R2 storage integration for upload microservice.

Implements S3-compatible API for R2 with:
- Multipart upload support for large files (>10MB)
- Circuit breaker pattern for resilience against R2 service failures
- Thread-pool execution for async compatibility with boto3
- Streaming upload support for memory efficiency

This is the migrated version from the monolith, adapted for the upload microservice
with service isolation and proper configuration from the core config module.
...
class StorageError(Exception):
    """Base storage error with HTTP status code support.
...
class StorageUnavailableError(StorageError):
    ...
    This error indicates that R2 storage has experienced too many failures
    and the circuit breaker is preventing further requests to allow recovery.
    """
    ...
    def __init__(
        ...
        super().__init__(message, code="STORAGE_UNAVAILABLE", status=503)
...
class UploadError(StorageError):
    ...
    def __init__(self, message: str, cause: Optional[str] = None) -> None:
        super().__init__(message, code="UPLOAD_FAILED", status=500)
        self.cause = cause
...
class MultipartError(StorageError):
    ...
    def __init__(
        ...
        super().__init__(message, code="MULTIPART_FAILED", status=500)
...
class StorageCircuitBreaker:
        ...
        async with self._lock:
            ...
            if self._state == StorageCircuitState.OPEN:
                if self._should_attempt_recovery():
                    ...
                    logger.info(
                        "Storage circuit breaker entering half-open state for recovery test",
                        extra={
                            "breaker_name": self._config.name,
    ...
    def _is_circuit_breaker_error(self, error: Exception) -> bool:
        ...
        if isinstance(error, StorageError):
            ...
            if error.code in ("INVALID_VARIANT", "OBJECT_NOT_FOUND"):
                return False
            return True
    ...
    def _record_failure(self, operation_name: str, error: Exception) -> None:
        ...
        if self._state == StorageCircuitState.HALF_OPEN:
            ...
            logger.warning(
                "Storage circuit breaker failure threshold exceeded, opening circuit",
                extra={
                    "breaker_name": self._config.name,
                    "failure_count": self._failure_count,
    ...
    def reset(self) -> None:
        ...
        """
        logger.info(
            "Storage circuit breaker manually reset",
            extra={
                "breaker_name": self._config.name,
    ...
    def force_open(self) -> None:
        ...
        """
        logger.info(
            "Storage circuit breaker manually opened",
            extra={
                "breaker_name": self._config.name,
                "previous_state": self._state.value,
            },
...
class R2StorageService:
    """Service for Cloudflare R2 storage operations using boto3.

    Features:
    - S3-compatible API for Cloudflare R2
    - Circuit breaker pattern for resilience against service failures
    - Multipart uploads for large files (automatic for files >10MB)
    - Thread-pool execution for async compatibility with boto3
    - Streaming support for memory-efficient large file handling

    The service uses lazy initialization to defer boto3 client creation
    until first use, allowing the service to start even if R2 credentials
    are not immediately available (useful for development).

    Example:
        service = get_r2_storage_service()

        # Upload encrypted file
        object_key = await service.upload_encrypted_file(
            workspace_id=uuid,
            gallery_id=uuid,
            asset_id=uuid,
            variant="original",
        ...
        data = await service.download_encrypted_file(
            workspace_id=uuid,
            gallery_id=uuid,
            asset_id=uuid,
            variant="original",
    ...
    # Maximum concurrent parts to upload (for memory management)
    ...
    def __init__(
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=10,
            thread_name_prefix="r2_storage_",
        )

        # Initialize circuit breaker with config from settings or custom
        cb_config = circuit_breaker_config or StorageCircuitBreakerConfig(
            failure_threshold=settings.CIRCUIT_BREAKER_FAILURE_THRESHOLD,
            recovery_time_ms=settings.CIRCUIT_BREAKER_RECOVERY_TIMEOUT * 1000,
            half_open_requests=settings.CIRCUIT_BREAKER_HALF_OPEN_REQUESTS,
            name="r2_storage",
        )
        self._circuit_breaker = StorageCircuitBreaker(cb_config)
    ...
    def _initialize_client(self) -> None:
        ...
        if not endpoint_url:
            if settings.APP_ENV == Environment.DEVELOPMENT:
                logger.warning(
                    "R2_ENDPOINT not set - R2 operations will fail. "
                    "This is expected in development without R2 access."
                )
                return
            raise StorageError(
                "R2_ENDPOINT is required for storage operations",
                code="R2_CONFIG_ERROR",
            )

        # Get secret key value safely
        secret_key = None
        if settings.R2_SECRET_ACCESS_KEY:
            secret_value = settings.R2_SECRET_ACCESS_KEY
            if hasattr(secret_value, "get_secret_value"):
                secret_key = secret_value.get_secret_value()
            else:
                secret_key = str(secret_value)
    ...
    def s3_client(self):
        ...
        if self._s3_client is None:
            raise StorageError(
                "R2 storage client not available - check R2_ENDPOINT configuration",
    ...
    def _build_object_key(
        ...
        Format: workspaces/{workspace_id}/galleries/{gallery_id}/{variant}/{asset_id}/{filename}.enc
        For library: workspaces/{workspace_id}/library/{variant}/{asset_id}/{filename}.enc

        Args:
            workspace_id: Workspace UUID
            gallery_id: Gallery UUID or None (for library)
            asset_id: Asset UUID
            variant: Media variant ('thumbnail', 'medium', 'preview', 'original')
        ...
        Raises:
            StorageError: If variant is invalid
        """
        # Validate variant
    ...
    async def upload_encrypted_file(
        ...
        try:
            ...
            raise UploadError(f"Upload failed: {e}", cause=str(e)) from e
    ...
    async def upload_bytes(
        ...
        async def _do_upload():
            ...
            await loop.run_in_executor(
                self.executor,
                lambda: self.s3_client.put_object(**params),
            )
            logger.debug(f"Uploaded bytes to R2: {key}", extra={"size": len(data)})
            return key
    ...
    async def upload_stream(
        ...
        """Upload a stream to R2, using multipart for large files.

        This method is useful for streaming uploads where the complete
        data is not available in memory at once.
        ...
        if content_length > self.MULTIPART_THRESHOLD:
            return await self._upload_stream_multipart(
                key=key,
                stream=stream,
        ...
        # For smaller files, read into memory and upload
        data = stream.read()
    ...
    async def download_encrypted_file(
        ...
        try:
            ...
            raise StorageError(f"Download failed: {e}", code="DOWNLOAD_FAILED") from e
    ...
    async def download_by_object_key(self, object_key: str) -> bytes:
        ...
        async def _do_download():
            ...
            response = await loop.run_in_executor(
                self.executor,
                lambda: self.s3_client.get_object(
                    Bucket=self.bucket_name, Key=key_with_enc
                ),
            )
            encrypted_data = await loop.run_in_executor(
                self.executor,
        ...
        try:
            ...
            error_code = e.response.get("Error", {}).get("Code", "Unknown")
            if error_code == "NoSuchKey":
                raise ObjectNotFoundError(key_with_enc) from e
            logger.error(f"Failed to download from R2: {e}")
            raise StorageError(f"Download failed: {e}", code="DOWNLOAD_FAILED") from e
        except BotoCoreError as e:
            logger.error(f"R2 client error: {e}")
    ...
    async def delete_file(
        ...
        try:
            ...
            raise StorageError(f"Deletion failed: {e}", code="DELETE_FAILED") from e
    ...
    async def delete_objects_bulk(
        ...
        S3/R2 allows deleting up to 1000 objects per request.

        Args:
            object_keys: List of object keys to delete
            quiet: If True, only return errors. If False, return all deleted keys.

        Returns:
            List of object keys that failed to delete (empty if all successful)
        ...
        # S3/R2 limit is 1000 objects per request
        batch_size = 1000
        ...
        for i in range(0, len(object_keys), batch_size):
            ...
            async def _do_delete():
                ...
                response = await loop.run_in_executor(
                    ...
                    lambda: self.s3_client.delete_objects(
                        Bucket=self.bucket_name,
                        Delete={"Objects": delete_objects, "Quiet": quiet},
                    ),
                )
            ...
            try:
                batch_errors = await self._execute_with_circuit_breaker(
                    _do_delete,
                    "delete_objects_bulk",
                )
                failed_keys.extend(batch_errors)
            except (StorageUnavailableError, StorageError):
                # On failure, mark all batch keys as failed
                failed_keys.extend(batch)

        if failed_keys:
            logger.warning(
                f"Failed to delete {len(failed_keys)} objects from R2",
                extra={"failed_count": len(failed_keys)},
            )
    ...
    async def file_exists(
        ...
        try:
            ...
            if error_code in ("404", "NoSuchKey"):
                return False
            raise StorageError(
                f"Failed to check file existence: {e}", code="CHECK_FAILED"
            ) from e

    async def object_exists(self, object_key: str) -> bool:
        """Check if an object exists in R2 by key.
        ...
        try:
            ...
            if error_code in ("404", "NoSuchKey"):
                return False
            raise StorageError(
                f"Failed to check object existence: {e}", code="CHECK_FAILED"
            ) from e
    ...
    async def generate_presigned_put_url(
        ...
        Args:
            ...
            expiry_seconds: URL expiry time in seconds (defaults to R2_SIGNED_URL_EXPIRY)

        Returns:
            Presigned URL for PUT request
        """
        if expiry_seconds is None:
            expiry_seconds = self._settings.R2_SIGNED_URL_EXPIRY
    ...
    async def create_multipart_upload(
        ...
        Raises:
            StorageUnavailableError: If circuit breaker is open
            MultipartError: If creation fails
        """
        ...
        async def _do_create():
            ...
            response = await loop.run_in_executor(
                self.executor,
                lambda: self.s3_client.create_multipart_upload(**params),
            )
            upload_id = response["UploadId"]
            logger.debug(
                f"Created multipart upload: {object_key}",
        ...
        try:
            ...
            raise MultipartError(f"Multipart create failed: {e}") from e
    ...
    async def upload_part(
        ...
        Raises:
            StorageUnavailableError: If circuit breaker is open
            MultipartError: If upload fails
        """
    ...
    async def complete_multipart_upload(
        ...
        Raises:
            StorageUnavailableError: If circuit breaker is open
            MultipartError: If completion fails
        """
        ...
        async def _do_complete():
            ...
            logger.info(
                f"Completed multipart upload: {object_key}",
                extra={
                    "etag": etag,
                    "upload_id": upload_id,
        ...
        try:
            ...
            raise MultipartError(
                f"Multipart complete failed: {e}", upload_id=upload_id
            ) from e
    ...
    async def abort_multipart_upload(
        ...
        Raises:
            MultipartError: If abort fails (logged as warning, not critical)
        """
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
            ...
            logger.info(
                f"Aborted multipart upload: {object_key}",
                extra={"upload_id": upload_id},
            )
        except (ClientError, BotoCoreError) as e:
            logger.warning(
                f"Failed to abort multipart upload (may already be complete): {e}",
                extra={"upload_id": upload_id, "object_key": object_key},
            )
    ...
    async def list_multipart_uploads(
        ...
        """List active multipart uploads for cleanup purposes.
        ...
        Returns:
            List of dicts with upload info (Key, UploadId, Initiated)
        """
        ...
        async def _do_list():
            ...
            response = await loop.run_in_executor(
                self.executor,
                lambda: self.s3_client.list_multipart_uploads(**params),
            )
    ...
    async def upload_large_file_multipart(
        ...
        Args:
            ...
            part_size: Size of each part (min 5MB, defaults to MULTIPART_PART_SIZE)
            metadata: Optional metadata dict
        ...
        if part_size is None:
            part_size = self._settings.MULTIPART_PART_SIZE

        upload_id = await self.create_multipart_upload(
            object_key, content_type, metadata
        )
    ...
    async def _upload_large_file_multipart_internal(
        self,
        object_key: str,
        data: bytes,
        content_type: str = "application/octet-stream",
    ) -> str:
        """Internal method for multipart upload used by upload_encrypted_file.

        Uses settings-based part size and doesn't accept external metadata.
        """
        return await self.upload_large_file_multipart(
            object_key=object_key,
            data=data,
            content_type=content_type,
            part_size=self._settings.MULTIPART_PART_SIZE,
    ...
    async def _upload_stream_multipart(
        self,
        key: str,
        stream: Union[io.BytesIO, io.BufferedReader],
        content_length: int,
        ...
        Internal method for streaming large file uploads.
        ...
        try:
            ...
            while bytes_uploaded < content_length:
            ...
            await self.complete_multipart_upload(key, upload_id, parts)
            return key
    ...
    async def health_check(self) -> dict[str, Any]:
        """Check R2 storage health and return status.
        ...
        health = {
            "healthy": self._circuit_breaker.is_healthy(),
            "circuit_breaker": {
        ...
        if self._circuit_breaker.is_healthy():
            try:
                loop = asyncio.get_event_loop()
                ...
                health["connectivity"] = "timeout"
                health["healthy"] = False
            except Exception as e:
                health["connectivity"] = f"error: {type(e).__name__}"
                health["healthy"] = False
    ...
    def shutdown(self) -> None:
        ...
        logger.info("R2 storage service shutdown complete")
...
def reset_r2_storage_service() -> None:
    """Reset singleton instance (for testing).

    Shuts down the executor before resetting.
    """
    global _r2_storage_service
    if _r2_storage_service is not None:
        _r2_storage_service.shutdown()
...
MULTIPART_THRESHOLD = R2StorageService.MULTIPART_THRESHOLD
MULTIPART_PART_SIZE = R2StorageService.MULTIPART_PART_SIZE
MAX_CONCURRENT_PARTS = R2StorageService.MAX_CONCURRENT_PARTS
