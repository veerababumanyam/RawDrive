...
class OnboardingSessionRepository:
    ...
    async def create(
        ...
        registration_data_json = json.dumps({})
    ...
    async def update(
        ...
        field_updates = {
            ...
            "registration_data": json.dumps(registration_data) if isinstance(registration_data, dict) else registration_data,
        ...
        async with pool.acquire() as conn:
            ...
            if next_step:
                row = await conn.fetchrow(
                    """
                    UPDATE onboarding_sessions
                    SET steps_completed = jsonb_set(json.dumps(steps_completed), $2, $3),
                        current_step = $4,
            ...
            row = await conn.fetchrow(
                ...
                SET status = 'completed',
                    ...
                    steps_completed = jsonb_set(
                        jsonb_set(json.dumps(steps_completed), '{workspace_setup}', 'true'),
                        '{completed}', 'true'
                    )
                WHERE session_id = $1
                RETURNING *
