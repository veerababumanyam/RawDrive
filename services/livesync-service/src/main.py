"""
LiveSync Service - Real-time Live Camera Sync Microservice.
...
Features:
- LiveSync mapping CRUD operations (folder-to-gallery mappings)
- LiveSync session management with real-time status
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "LiveSync Service started successfully",
        extra={
    ...
    # Shutdown
    logger.info("Shutting down LiveSync Service...")
    ...
    logger.info("LiveSync Service shutdown complete")
...
async def drain_active_sessions():
    ...
    while time.time() - start < timeout:
        ...
        if active_sessions == 0:
            logger.info("All livesync sessions drained successfully")
            return
...
app = FastAPI(
    title="RawDrive LiveSync Service",
    description="""
High-performance microservice for Live Camera Sync.

## Features

- **LiveSync Mappings**: Create and manage folder-to-gallery sync mappings
- **LiveSync Sessions**: Real-time sync session management with status tracking
...
- **KEDA Autoscaling**: Scales based on active livesync sessions and upload queue depth
...
Connect to `/api/v1/livesync/ws` with JWT token for real-time sync updates.
Supports the `livesync-v1` subprotocol.
    """,
    version=settings.SERVICE_VERSION,
    lifespan=lifespan,
