...
class InvitationDraftService:
    ...
    async def create_draft(
        ...
        draft_count = await redis_client.scard(index_key)
        ...
        await redis_client.setex(draft_key, DRAFT_TTL_SECONDS, json.dumps(draft_record))

        # Add to index
        await redis_client.sadd(index_key, draft_id)
        await redis_client.expire(index_key, DRAFT_TTL_SECONDS)
    ...
    async def update_draft(
        ...
        """
        # Use redis_client instance

        draft_key = self._get_draft_key(workspace_id, user_id, draft_id)

        # Get existing draft
        existing = await redis_client.get(draft_key)
        if not existing:
            return False
    ...
    async def list_drafts(
        ...
        index_key = self._get_index_key(workspace_id, user_id)
        draft_ids = await redis_client.smembers(index_key)
        ...
        for draft_id in draft_ids:
            ...
            if draft:
                ...
                await redis_client.srem(index_key, draft_id)
