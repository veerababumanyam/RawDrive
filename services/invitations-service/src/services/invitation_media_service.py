...
from src.models.invitation_media import MediaType
from src.repositories.invitation_media_repository import (
...
# Storage Service: For quota management - TODO: Implement or create HTTP client to backend
# For now, quota checks are optional
try:
    from src.services.storage_service import StorageService, get_storage_service
except ImportError:
    logger.warning("StorageService not available - quota checks will be skipped")
    class StorageService:
        async def check_upload_allowed(self, *args, **kwargs):
            return True, None  # Allow uploads if service unavailable
