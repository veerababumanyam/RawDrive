...
class TaskQueueService:
    ...
    async def enqueue(
        ...
        # Store task data using Redis client methods
        ...
        await redis_client.zadd(QUEUE_KEY, {task_id: score})
    ...
    async def _dequeue(self) -> Task | None:
        ...
        results = await redis_client.zrangebyscore(QUEUE_KEY, "-inf", str(now), start=0, num=1)

        if not results:
            # Check for any immediate tasks (negative scores)
            results = await redis_client.zrangebyscore(QUEUE_KEY, "-inf", "0", start=0, num=1)
        ...
        removed = await redis_client.zrem(QUEUE_KEY, task_id)
        if not removed:
            return None  # Another worker got it

        await redis_client.sadd(PROCESSING_SET, task_id)
    ...
    async def _complete_task(self, task: Task, result: Any = None) -> None:
        ...
        await redis_client.srem(PROCESSING_SET, task.task_id)
    ...
    async def _fail_task(self, task: Task, error: str) -> None:
        ...
        if task.retries < task.max_retries:
            ...
            await redis_client.zadd(QUEUE_KEY, {task.task_id: float(execute_at.timestamp())})
            await redis_client.srem(PROCESSING_SET, task.task_id)
            ...
            await redis_client.srem(PROCESSING_SET, task.task_id)
            await redis_client.lpush(DEAD_LETTER_QUEUE, task.task_id)
    ...
    async def get_queue_stats(self) -> dict[str, int]:
        ...
        failed = await redis_client.llen(DEAD_LETTER_QUEUE)
    ...
    async def retry_failed_task(self, task_id: str) -> bool:
        ...
        await redis_client.zadd(QUEUE_KEY, {task_id: float(-task.priority.value)})
        await redis_client.lrem(DEAD_LETTER_QUEUE, 1, task_id)
...
async def handle_queue_rsvp_for_digest(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    await redis_client.rpush(digest_key, json.dumps(rsvp_data))
    # Set expiry to 48 hours (in case digest job fails, data persists)
    await redis_client.expire(digest_key, 172800)
...
async def handle_send_rsvp_digest(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    rsvps_raw = await redis_client.lrange(digest_key, 0, -1)
    if not rsvps_raw:
