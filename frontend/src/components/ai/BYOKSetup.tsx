    ...
    <div className="rounded-xl border border-border-default bg-surface-raised p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Provide your own Google Gemini API key to enable AI features. Your key
      ...
      {config?.configured && (
        <div className="flex items-center gap-3 rounded-lg bg-surface-sunken px-4 py-3">
          ...
          <span className="text-sm text-text-primary">
            Key configured: {config.key_masked}
          </span>
      ...
      <div className="flex gap-3">
        <button
          ...
          className="rounded-lg bg-accent px-5 py-2.5 text-sm font-medium text-text-inverse hover:bg-accent/90 disabled:opacity-50 min-h-[44px]"
        >
      ...
      {validResult && (
        <div
          className={`rounded-lg px-4 py-3 text-sm ${validResult.valid ? "bg-feedback-success/10 text-feedback-success" : "bg-feedback-error/10 text-feedback-error"}`}
        >
          {validResult.valid
            ? "Key is valid and working."
            : `Key validation failed: ${validResult.error}`}
        </div>
      )}
    </div>
  );
