...
export function DuplicateComparison({ group, onResolve, onDismiss }: DuplicateComparisonProps) {
  const [selected, setSelected] = useState<Set<string>>(
    new Set(group.members.filter((m) => m.is_representative).map((m) => m.asset_id))
  );
      ...
      {group.status === "pending" && (
        <div className="flex gap-2 mt-4">
          <button
            ...
            className="flex-1 rounded-lg bg-accent px-4 py-2 text-sm font-medium text-white hover:bg-accent/90 disabled:opacity-50 min-h-[44px]"
          >
