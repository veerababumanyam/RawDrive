"use client";
/** * GalleryLockedShell — the non-broken "you cannot see this yet" state for a * gated public gallery (S4-G3, integration audit 2026-05-31). * * Rendered when GET /public/galleries/{slug} returns the locked shell * ({ access_gated: true, access_mode, has_password }) for a private / * invite-only gallery viewed without a valid session, OR for an expired * gallery. The viewer must NOT fall through to an empty asset grid — that * reads as "the photographer published nothing" instead of "this is locked". * * Three variants: * - "private" → access_mode === "private": requires a share link. * - "invite-only" → access_mode === "invite-only": requires an invite. * - "expired" → the gallery's expiry window has passed. * * Token-only styling, renders identically across all three themes (no * theme-specific overrides). The single CTA, when present, is a real 44px * touch target with a token focus ring. */ type LockedVariant =
  "private" | "invite-only" | "expired";
...
export function GalleryLockedShell({
  variant,
  title,
  brandName,
  logoUrl,
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface px-4">
      ...
      <div className="surface-panel w-full max-w-sm p-8 text-center">
        ...
        {brandName && (
          <p className="mb-2 text-xs uppercase text-text-tertiary">
            {brandName}
          </p>
        )}{" "}
