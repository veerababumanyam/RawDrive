      ...
      >
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-lg glass-surface border border-glass-border text-sm text-accent placeholder:text-tertiary focus:outline-none focus:ring-1 focus:ring-accent"
          />
          <button
            type="submit"
            disabled={!newComment.trim()}
            className="px-4 py-2 rounded-lg bg-accent text-text-inverse text-sm font-medium disabled:opacity-50"
          >
            Send
          </button>
        </div>
  ...
  return (
    <div className="glass-surface rounded-xl p-3 border border-glass-border">
      ...
      <div className="flex items-center gap-3 mt-2">
        ...
        {comment.is_resolved && (
          <span className="text-xs text-feedback-success">✓ Resolved</span>
        )}
      </div>
    </div>
  );
