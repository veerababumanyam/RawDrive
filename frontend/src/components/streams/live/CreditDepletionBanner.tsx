...
const COPY: Record<
  Exclude<DepletionPhase, "ok" | "ended">,
  { label: string; tone: string }
> = {
  warn5: {
    label:
      "Low balance: about 5 minutes of stream credit remaining. Recharge to avoid interruption.",
    tone: "border-feedback-warning/30 bg-feedback-warning/10 text-feedback-warning",
  },
  warn1: {
    label:
      "Low balance: about 1 minute of stream credit remaining. Recharge now.",
    tone: "border-feedback-warning/50 bg-feedback-warning/15 text-feedback-warning",
  },
  warn30: {
    label:
      "Critical: ~30 seconds of stream credit left. Stream will auto-end shortly.",
    tone: "border-feedback-error/40 bg-feedback-error/15 text-feedback-error",
  },
  grace: {
    label:
      "Credits depleted — 30-second grace window to recharge before auto-end.",
    tone: "border-feedback-error/60 bg-feedback-error/20 text-feedback-error",
  },
};

export function CreditDepletionBanner({
  phase,
  remainingSec,
}: CreditDepletionBannerProps) {
