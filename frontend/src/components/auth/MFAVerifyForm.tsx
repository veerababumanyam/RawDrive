...
export function MFAVerifyForm() {
  ...
  useEffect(() => {
    ...
    const flow = window.sessionStorage.getItem(MFA_FLOW_KEY);
    if (stored) {
      setMfaToken(stored);
      return;
    }
    if (flow === "oauth") {
      // Empty string signals "ready to verify, but the server reads the
      // token from the HttpOnly mfa_token cookie".
      setMfaToken("");
      return;
    }
