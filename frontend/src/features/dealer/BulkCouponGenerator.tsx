...
interface Props { onGenerate: (data: { pattern: string; count: number; coupon_type: string; discount_value: number; valid_from: string }) => Promise<{ generated_count: number; codes: string[] }>; isDealer: boolean; }
...
export default function BulkCouponGenerator({ onGenerate, isDealer }: Props) {
  ...
  const samples = useMemo(() => pattern.includes("X") ? [generateSample(pattern), generateSample(pattern), generateSample(pattern)] : [], [pattern]);

  const handleGenerate = async () => {
    if (!pattern.includes("X")) return;
    setGenerating(true);
    try {
      const res = await onGenerate({ pattern, count, coupon_type: couponType, discount_value: discountValue, valid_from: validFrom });
      setResult(res);
    } catch { setResult(null); } finally { setGenerating(false); }
  };
  ...
  return (
    <div className="glass-card p-6 space-y-4 max-w-lg">
      ...
      <div>
        ...
        {samples.length > 0 && <div className="mt-2 flex gap-2">{samples.map((s, i) => <span key={i} className="px-2 py-1 bg-surface-sunken rounded text-xs font-mono text-text-secondary">{s}</span>)}</div>}
      </div>
      ...
      {result && (
        <div className="glass-card p-4 rounded-xl bg-surface-container space-y-2">
          ...
          <div className="flex flex-wrap gap-1 max-h-40 overflow-y-auto">{result.codes.map(c => <span key={c} className="px-2 py-0.5 bg-surface-sunken rounded text-xs font-mono text-text-primary">{c}</span>)}</div>
