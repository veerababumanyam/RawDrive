...
export function getBrowserE2EEUploadBlockReason(file: File): string | null {
  ...
  // Exotic/proprietary RAW, TIFF, and any other `image/x-*` still image remain
  // desktop-only. The `image/x-*` glob is now reached ONLY for formats the
  // in-browser decoders cannot handle, because every browser-decodable family
  // (including CR3) returned null above.
  if (
