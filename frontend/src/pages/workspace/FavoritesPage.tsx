...
const FavoritesPage: React.FC = () => {
  ...
  const handleTogglePin = async (gallery: GalleryListItem) => {
     ...
     try {
         // Since this is Favorites page, toggle pin essentially means unpinning
         await galleryService.unpinGallery(workspace.workspace_id, gallery.gallery_id);
         addToast({
             variant: 'success',
             message: `Removed "${gallery.title}" from favorites`,
         });
         refetch();
     } catch (err) {
         addToast({
    ...
    >
        ...
        <div>
          ...
          <p className="text-text-secondary mt-1 ml-11">
            {loading ? (
              ...
              <>
                {galleries.length} pinned galleries
              </>
            )}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <motion.div variants={staggerItem} className="text-center py-20 card-glass rounded-3xl border border-white/10">
             ...
             <h3 className="text-lg font-medium text-text-primary">No favorites yet</h3>
             <p className="text-text-secondary mt-2 max-w-sm mx-auto">
               Pin your most important galleries for quick access. They will appear here.
             </p>
             <AppButton 
                className="mt-6"
                variant="outline"
                    ...
                    >
                       ...
                       <td className="px-4 py-3 text-right">
                          <button
                             onClick={(e) => {
                                 e.stopPropagation();
                                 handleTogglePin(gallery);
                             }}
                            className="p-2 rounded-lg hover:bg-surface-hover text-text-tertiary hover:text-red-500 transition-colors"
                            title="Unpin"
                          >
                             <Trash2 size={16} /> 
                          </button>
