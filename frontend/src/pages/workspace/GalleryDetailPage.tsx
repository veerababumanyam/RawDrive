...
const GalleryDetailPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Action Buttons - Enhanced with glass card */}
      <div className="card-glass rounded-2xl p-3 sm:p-4 flex items-center justify-end gap-2 flex-wrap">
        <PublishButton
          gallery={gallery}
          onPublish={async () => {
            if (!workspace?.workspace_id || !galleryId) return;
        ...
        <AppButton variant="outline" onClick={() => setShowUpload(!showUpload)} className="hover:-translate-y-0.5 transition-all">
          <Upload size={16} className="mr-2" />
          Upload
        </AppButton>
        <AppButton variant="outline" className="hover:-translate-y-0.5 transition-all">
      ...
      {/* Upload Section - Enhanced with glass effect */}
      {showUpload && (
        <AppCard padding="md" variant="glass" className="card-glass border border-white/10 dark:border-white/5">
          <GalleryUpload
        ...
        <BulkActionBar
            ...
            try {
              ...
              addToast({
                ...
                action: {
                  ...
                  onClick: async () => {
                    try {
                      // Restore assets by setting visible = TRUE
          ...
          onBulkDownload={async (assetIds) => {
            try {
              ...
              addToast({ message: 'Failed to download photos', variant: 'error' });
            }
          }}
        />
      )}

      {/* Toolbar - View mode, search, and select all only */}
      <GalleryToolbar
        viewMode={viewMode}
        filter={filter}
        searchQuery={searchQuery}
      ...
      {/* Filter Bar - Advanced filtering with combined filters - Enhanced with glass */}
