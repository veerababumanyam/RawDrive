...
import { PROJECT_STATUS_CLASS, PROJECT_STATUSES, getProjectStatusLabel } from "@/lib/crm-taxonomy";

const PROJECT_TYPES = ["wedding", "pre_wedding", "portrait", "corporate", "commercial", "newborn", "family", "other"];
...
export default function ProjectsPage() {
  ...
  const summary = useMemo(() => {
    ...
    const bookedValue = projects.reduce((sum, project) => sum + (project.booked_value_paisa || 0), 0);
    const balanceDue = projects.reduce((sum, project) => sum + (project.balance_due_paisa || 0), 0);
    const nextActions = projects.filter((project) => Boolean(project.next_action));
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-medium text-accent-primary">Studio Projects</p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-text-primary">Project Board</h1>
          <p className="mt-2 max-w-3xl text-sm text-text-secondary">
            One job record for the client, shoot date, documents, billing, gallery delivery, and next action.
          </p>
        </div>
        <button
      ...
      <section className="grid grid-cols-1 gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-border-default bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-wide text-text-tertiary">Active projects</p>
          <p className="mt-2 text-2xl font-semibold text-text-primary">{summary.activeProjects.length}</p>
        </div>
        <div className="rounded-2xl border border-border-default bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-wide text-text-tertiary">Booked value</p>
          <p className="mt-2 text-2xl font-semibold text-text-primary">{formatPaisa(summary.bookedValue)}</p>
        </div>
        <div className="rounded-2xl border border-border-default bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-wide text-text-tertiary">Balance due</p>
          <p className="mt-2 text-2xl font-semibold text-text-primary">{formatPaisa(summary.balanceDue)}</p>
        </div>
        <div className="rounded-2xl border border-border-default bg-surface-raised p-4">
          <p className="text-xs uppercase tracking-wide text-text-tertiary">Next actions</p>
          <p className="mt-2 text-2xl font-semibold text-text-primary">{summary.nextActions.length}</p>
        </div>
      </section>

      {showCreate && (
        <section className="rounded-2xl border border-border-default bg-surface-raised p-6">
          <h2 className="text-lg font-semibold text-text-primary">New Studio Project</h2>
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <input
              ...
              onChange={(event) => setForm({ ...form, expected_value_rupees: Number(event.target.value) || 0 })}
              className="rounded-xl border border-border-default bg-surface-sunken px-4 py-2.5 text-text-primary"
            ...
            <input
              ...
              onChange={(event) => setForm({ ...form, booked_value_rupees: Number(event.target.value) || 0 })}
              className="rounded-xl border border-border-default bg-surface-sunken px-4 py-2.5 text-text-primary"
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border-default p-10 text-center">
          ...
          <p className="mt-1 text-sm text-text-secondary">Create a project from a client or convert an inquiry into a booked job.</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-text-secondary">
                    {[project.contact_name, formatDate(project.event_date), project.city].filter(Boolean).join(" · ")}
                  </p>
                </div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
                <div>
                  ...
                  <p className="font-medium text-text-primary">{formatPaisa(project.expected_value_paisa || 0)}</p>
                </div>
                <div>
                  <p className="text-text-tertiary">Booked</p>
                  <p className="font-medium text-text-primary">{formatPaisa(project.booked_value_paisa || 0)}</p>
                </div>
                <div>
                  <p className="text-text-tertiary">Balance</p>
                  <p className="font-medium text-text-primary">{formatPaisa(project.balance_due_paisa || 0)}</p>
                </div>
              </div>
              {project.next_action && (
                <p className="mt-4 rounded-xl bg-accent-subtle px-3 py-2 text-sm text-accent-primary">Next: {project.next_action}</p>
              )}
            </Link>
          ))}
        </section>
