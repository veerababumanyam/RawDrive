  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex gap-1 rounded-xl bg-surface-sunken p-1">
        {tabs.map((tab) => {
          const isActive = pathname === tab.href || (tab.href !== "/ai" && pathname?.startsWith(tab.href));
          return (
            <Link
              key={tab.href}
              href={tab.href}
              className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors min-h-[44px] flex items-center ${
                isActive
