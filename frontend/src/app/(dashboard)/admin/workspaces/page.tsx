...
export default function AdminWorkspacesPage() {
  ...
  if (loading) return <div className="max-w-7xl mx-auto space-y-8 p-8"><p className="text-gray-500">Loading workspaces...</p></div>;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-2 font-body text-sm">Storage, subscriptions, and state metrics per workspace.</p>
      </div>

      {workspaces.length === 0 ? (
        <div className="text-center py-16 text-gray-500">No workspaces found.</div>
      ) : (
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {workspaces.map((ws) => (
                  <tr key={ws.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-6 py-5 text-sm text-gray-400">{ws.owner_name}</td>
                    <td className="px-6 py-5 text-sm text-gray-400">{ws.state_name || "—"}</td>
                    <td className="px-6 py-5 text-sm text-secondary font-medium">{formatBytes(ws.storage_used_bytes)}</td>
                    <td className="px-6 py-5 text-sm text-gray-400">{ws.asset_count}</td>
                    <td className="px-6 py-5 text-sm font-medium text-primary">{ws.subscription_tier || "Free"}</td>
                    <td className="px-6 py-5 text-xs text-gray-600 font-label">{new Date(ws.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
