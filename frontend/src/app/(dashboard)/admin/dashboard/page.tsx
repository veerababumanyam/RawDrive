...
import { Users, LayoutGrid, TrendingUp, Server } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {hint && (
          <p className="mt-1 text-[11px] text-text-tertiary/80">{hint}</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          hint="Unique non-deleted users in the platform"
        />
        <StatCard
          icon={LayoutGrid}
          label="Active Workspaces"
          value={loading ? "..." : (data?.activeWorkspaces ?? "--")}
          hint="Workspaces with status=active (excludes suspended/deleted)"
        />
        <StatCard
          icon={TrendingUp}
          label="Monthly Revenue"
          value={loading ? "..." : (data?.monthlyRevenue ?? "--")}
          hint="MRR this calendar month"
        />
        <StatCard
          icon={Server}
          label="System Status"
          value={loading ? "..." : (data?.systemStatus ?? "--")}
          hint="Healthy when API p95 latency < 500ms"
        />
      </div>
