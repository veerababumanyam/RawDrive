...
func (r *AssetDerivativeRepo) Upsert(ctx context.Context, d *AssetDerivative) error {
	...
	_, err := r.pool.Exec(ctx,
		`INSERT INTO asset_derivatives (id, asset_id, variant, storage_key, width, height, size_bytes, format, created_at)
		 VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
		 ON CONFLICT (asset_id, variant) DO UPDATE
		 SET storage_key = $4, width = $5, height = $6, size_bytes = $7, format = $8, created_at = $9`,
		d.ID, d.AssetID, d.Variant, d.StorageKey, d.Width, d.Height, d.SizeBytes, d.Format, d.CreatedAt,
	)
	if err != nil {
...
func (r *AssetDerivativeRepo) ListByAsset(ctx context.Context, assetID uuid.UUID) ([]AssetDerivative, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, asset_id, variant, storage_key, width, height, size_bytes, format, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.ID, &d.AssetID, &d.Variant, &d.StorageKey, &d.Width, &d.Height, &d.SizeBytes, &d.Format, &d.CreatedAt); err != nil {
...
func (r *AssetDerivativeRepo) GetByVariant(ctx context.Context, assetID uuid.UUID, variant string) (*AssetDerivative, error) {
	...
	err := r.pool.QueryRow(ctx,
		...
		assetID, variant,
	).Scan(&d.ID, &d.AssetID, &d.Variant, &d.StorageKey, &d.Width, &d.Height, &d.SizeBytes, &d.Format, &d.CreatedAt)
	if err != nil {
