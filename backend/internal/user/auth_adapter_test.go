...
func TestAuthAdapter_Create_TranslatesPhoneTaken(t *testing.T) {
	...
	_, err := user.NewAuthAdapter(svc).Create(
		ctx, "first@example.com", "Secret12345", "First", "+919876543210", 29,
	)
	...
	_, err = user.NewAuthAdapter(svc).Create(
		ctx, "second@example.com", "Secret12345", "Second", "+919876543210", 29,
	)
	require.Error(t, err)
...
func TestAuthAdapter_Create_PropagatesOtherErrors(t *testing.T) {
	...
	// Seed an email.
	_, err := user.NewAuthAdapter(svc).Create(
		ctx, "dup@example.com", "Secret12345", "First", "", 29,
	)
	require.NoError(t, err)
	...
	_, err = user.NewAuthAdapter(svc).Create(
		ctx, "dup@example.com", "Secret12345", "Dup", "", 29,
	)
	require.Error(t, err)
