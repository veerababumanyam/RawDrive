...
// resolveWorkspaceSubdomain reads the legacy workspace scope token (migration
// 121 shape: <biz-slug>-<biz-code>) from the request. Two sources, checked in
// priority order:
//
//  1. `?ws=` query parameter — retained for older session-scoped links and
//     internal callers. New public gallery URLs do not generate it.
...
// Deprecated: public studio subdomain landings are disabled. Canonical public
// gallery URLs are https://rawdrive.in/g/<slug>.
func (h *PublicGalleryHandler) GetStudioLanding(w http.ResponseWriter, r *http.Request) {
	http.Error(w, `{"error":"studio subdomains are disabled"}`, http.StatusGone)
}
...
// Deprecated with public studio subdomain landings.
