...
CREATE INDEX idx_moderation_queue_status ON moderation_queue(status, created_at DESC);
CREATE INDEX idx_moderation_queue_workspace ON moderation_queue(workspace_id);
...
CREATE INDEX idx_moderation_rules_type ON moderation_rules(content_type) WHERE is_active = true;
...
INSERT INTO moderation_rules (content_type, rule_type, config) VALUES
('message', 'keyword', '{"keywords": ["spam", "scam", "fraud"], "action": "flag"}'),
('freelancer_listing', 'keyword', '{"keywords": ["spam", "fake"], "action": "flag"}'),
('gear_listing', 'keyword', '{"keywords": ["stolen", "fake", "counterfeit"], "action": "flag"}');
