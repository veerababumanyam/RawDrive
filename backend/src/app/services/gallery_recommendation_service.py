"""Gallery Design Recommendation Service.
...
class GalleryRecommendationService:
    ...
    def __init__(self, repository: Optional[Any] = None, redis_client: Optional[Any] = None):
        """Initialize recommendation service.

        Args:
            repository: GalleryRepository instance for data access
    ...
    async def generate_recommendations(
        ...
        all_assets = await self.repository.get_gallery_assets(
    ...
    async def _select_sample(self, all_assets: list[Any]) -> list[Any]:
        ...
        Returns:
            List of 10 (or fewer) representative Asset objects
        """
        ...
        embeddings = [
            ...
            if hasattr(asset, "clip_embedding") and asset.clip_embedding is not None
        ]
        ...
        sorted_assets = sorted(
            all_assets,
            key=lambda a: a.date_taken or a.created_at,
        )
    ...
    async def _analyze_sample(self, sample: list[Any]) -> RecommendationMetrics:
...
def get_gallery_recommendation_service(
    ...
    Args:
        repository: GalleryRepository instance
        redis_client: Redis client for caching

    Returns:
        GalleryRecommendationService instance
    """
