...
from app.utils.crypto import hash_token as _hash_token
...
class SessionService:
    ...
    async def create_session_with_id(
        ...
        """Create session with pre-generated ID and custom TTL.

        Used by AuthService to create session atomically with real refresh token.
        The session_id is pre-generated by the caller before token generation,
        ensuring the session_id is included in the JWT token claims.
        ...
        Args:
            ...
            ttl_seconds: Override default TTL (if None, uses settings default)
        ...
        # Store session in Redis with custom TTL
        await redis.setex(
            _session_key(session_id),
            effective_ttl,
            json.dumps(session.to_dict()),
        ...
        try:
            ...
            # Rollback Redis on PostgreSQL failure
            await redis.delete(_session_key(session_id))
            await redis.srem(_user_sessions_key(user_id), str(session_id))
            logger.error(
                "Failed to create session in PostgreSQL, rolled back Redis",
                extra={
                    "error_type": type(e).__name__,
                    "error_message": str(e),
                    "user_id": str(user_id),
        ...
        logger.info(
            "Session created with pre-generated ID",
            extra={"session_id": str(session_id), "user_id": str(user_id), "ttl_seconds": effective_ttl}
        )
        return session
