...
async def repair_all():
    ...
    from app.config.test_constants import TierUsers
    
    await init_postgres_pool()
    pool = await get_postgres_pool()
    
    # Get all tier users
    users_to_fix = TierUsers.all()
    ...
    for user in users_to_fix:
        ...
        print(f"  Workspace found: {ws['name']}")
        ...
        try:
            ...
            # Init is likely harmless if called multiple times or already init by other parts? 
            # safe to call it if checking _pool inside
            await init_redis_client() 
            redis = await get_redis_client()
            await redis.delete(f"perms:{user.user_id}:{ws_id}")
            print("  Cleared permission cache.")
        except Exception as e:
            print(f"  Failed to clear cache (non-critical): {e}")

    print("\n✅ Repair completed for all test users.")
