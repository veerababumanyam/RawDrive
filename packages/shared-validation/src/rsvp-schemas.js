...
export const customAnswersSchema = z
    ...
    .refine((obj) => Object.keys(obj).length <= MAX_CUSTOM_ANSWERS, `Cannot have more than ${MAX_CUSTOM_ANSWERS} custom answers`)
    .optional()
    .default({});
/**
 ...
 */
export function createPartySizeSchema(maxSize) {
    return z
        .number()
        .int('Party size must be a whole number')
        .min(1, 'Party size must be at least 1')
 ...
 */
export function validatePartyNamesMatchSize(partyNames, partySize) {
 ...
 */
export function isRSVPDeadlinePassed(deadline) {
    if (!deadline) {
        return false;
    }
    try {
